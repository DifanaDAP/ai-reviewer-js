...
`
  ...
  const titleRegex = /^(feat|fix|docs|style|refactor|perf|test|chore|revert|ci|build|temp)(\(.+\))?: .+/
  const isTitleValid = titleRegex.test(prTitle)
  const isDescriptionValid = prDescription.length > 10
        ...
        try {
          ...
          if (jsonMatch) {
            try {
              ...
              if (analysis.security) allSecurityIssues.push(...analysis.security)
              if (analysis.performance) allPerformanceIssues.push(...analysis.performance)
              if (analysis.style) allCodeStyleIssues.push(...analysis.style)
            } catch (e) {
              warning(`Failed to parse JSON summary for ${filename}: ${e}`)
            }
          }
